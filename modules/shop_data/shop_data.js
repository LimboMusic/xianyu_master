...
const IMAGE_URL_CLASS_NAME = '.carouselItem--jwFj0Jpa img';
...
async function gotoShopPage(page, className = FIRST_CARD_CLASS_NAME) {
    ...
    if (await elementLocator.count() === 0) {
        console.log(`Element with className "${className}" not found on page ${page.url()}`);
        return null;
    }
    ...
    try {
        ...
        if (error.message.includes('Timeout') || error.message.includes('waitForEvent')) {
            console.log(`Warning: Failed to open shop page (timeout waiting for page event): ${error.message}`);
            return 'error page';
        }
        return 'error page';
    }
    ...
    try {
        ...
        if (error.message.includes('Timeout') || error.message.includes('waitForLoadState')) {
            console.log(`Warning: Page load timeout for shop page, skipping this item: ${error.message}`);
            try {
                await newPage.close();
            } catch (closeError) {
                // 忽略关闭错误
...
async function getImageUrls(page) {
    try {
        ...
        // 获取所有媒体URL (图片和视频)
        const mediaLocators = page.locator(`${IMAGE_URL_CLASS_NAME}, ${VIDEO_URL_CLASS_NAME}`);
...
async function gotoNextPage(page) {
    await page.locator('.search-pagination-arrow-container--lt2kCP6J').nth(1).click();
    await sleep(5000)
}
...
async function extractItemId(url) {
    try {
        ...
        console.log(`Warning: Failed to extract item ID from URL ${url}: ${error.message}`);
        return url;
    }
}

export { gotoShopPage, getIntroText, getReviewAndWantNumber, getLinkDescription, getUserName, getImageUrls, gotoNextPage, extractItemId };